...
// --- Sub-components for Hook Safety & Responsiveness ---
...
const OrbitingPerk = ({ item, idx, dx, dy, isMobile }: { item: any, idx: number, dx: any, dy: any, isMobile: boolean }) => {
  ...
  return (
    <motion.div
      ...
      className="absolute p-3 sm:p-4 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-white flex flex-col items-center gap-1 sm:gap-2 group cursor-pointer"
      whileHover={{ scale: 1.15, zIndex: 30 }}
    >
      <div className={`p-1.5 sm:p-2 rounded-xl bg-slate-50 ${item.color}`}>
        <item.icon size={isMobile ? 18 : 24} />
      </div>
      <span className="text-[7px] sm:text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 transition-colors">
        {item.label}
      </span>
    </motion.div>
  );
...
const InteractivePhone = ({ rotateX, rotateY, isMobile }: { rotateX: any, rotateY: any, isMobile: boolean }) => {
  return (
    <motion.div 
      ...
      className="relative z-10 w-40 sm:w-56 h-[320px] sm:h-[450px] bg-slate-900 rounded-[2rem] sm:rounded-[3rem] border-[6px] sm:border-[10px] border-slate-800 shadow-[0_40px_100px_rgba(0,0,0,0.3)] overflow-hidden"
    ...
    >
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-t from-brand-500/80 via-transparent to-transparent flex flex-col justify-end p-4 sm:p-8 text-white">
        <div className="flex items-center gap-1 sm:gap-2 mb-1 sm:mb-2">
          <Zap size={isMobile ? 12 : 16} fill="white" className="text-white" />
          <span className="text-[10px] sm:text-xs font-black uppercase tracking-tighter">Live Mentorship</span>
        </div>
      </div>
    </motion.div>
  );
...
const SuccessBadge = ({ dx, dy, isMobile }: { dx: any, dy: any, isMobile: boolean }) => {
  const xTransform = useTransform(dx, [-500, 500], [-8, 8]);
  const yTransform = useTransform(dy, [-500, 500], [-8, 8]);
  ...
  return (
    <motion.div 
      ...
      className="absolute -bottom-10 sm:-bottom-8 right-0 sm:right-10 bg-white p-3 sm:p-5 rounded-2xl shadow-2xl border border-slate-100 z-20 flex items-center gap-3 sm:gap-4"
    >
      <div className="p-2 sm:p-3 bg-brand-50 text-brand-600 rounded-xl">
        <Sparkles size={isMobile ? 18 : 24} />
      </div>
      <div>
        <div className="text-base sm:text-xl font-extrabold text-slate-900">AIR 1 Strategies</div>
        <div className="text-[8px] sm:text-[10px] text-brand-500 font-black uppercase tracking-widest">Available Now</div>
      </div>
    </motion.div>
  );
};

const OrbitingParticle = ({ i, dx, dy }: { i: number, dx: any, dy: any }) => {
  const xTransform = useTransform(dx, [-500, 500], [(i - 5) * 50, (i - 5) * -50]);
  const yTransform = useTransform(dy, [-500, 500], [(i - 5) * 50, (i - 5) * -50]);
...
const StudentJoinPage = () => {
  ...
  // Grid Transforms (must be top-level)
  const gridX = useTransform(dx, [-500, 500], [-15, 15]);
  const gridY = useTransform(dy, [-500, 500], [-15, 15]);
    ...
    <main className="min-h-screen bg-[#FAFAFA] selection:bg-brand-500 selection:text-white pt-24 font-sans">
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
          ...
          >
            <div className="flex-1 space-y-8 z-20">
              ...
              >
                <Rocket size={16} />
                <span className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">Join India's Most Elite Student Circle</span>
              </motion.div>
              
              <motion.h1 
                variants={fadeUpVariant}
                className="text-4xl sm:text-5xl lg:text-[72px] font-bold text-slate-900 leading-[1.1] tracking-tight text-center lg:text-left"
              >
              ...
              <motion.p 
                variants={fadeUpVariant}
                className="text-base sm:text-lg text-slate-500 max-w-xl leading-relaxed font-medium text-center lg:text-left mx-auto lg:mx-0"
              >
              ...
              <motion.div 
                variants={fadeUpVariant}
                className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 sm:gap-5 pt-4"
              >
                <a 
                  href="#download" 
                  className="w-full sm:w-auto group relative overflow-hidden rounded-full bg-brand-500 text-white px-8 py-4 font-bold shadow-xl transition-all hover:shadow-[0_0_40px_rgba(251,146,60,0.4)] hover:-translate-y-1 flex items-center justify-center gap-2"
                >
                ...
                <a 
                  href="#form" 
                  className="w-full sm:w-auto rounded-full bg-white text-slate-700 px-8 py-4 font-bold border border-slate-200 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm flex items-center justify-center gap-2"
                >
            ...
            >
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                ...
                {!isMobile && [...Array(10)].map((_, i) => (
                  <OrbitingParticle key={i} i={i} dx={dx} dy={dy} />
                ))}
              </div>

              {/* Success Badge */}
              <SuccessBadge dx={dx} dy={dy} isMobile={isMobile} />
            </motion.div>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <motion.div 
                key={idx} 
                whileHover={{ y: -8 }}
                className="space-y-4 p-6 sm:p-8 bg-slate-50/50 hover:bg-white hover:shadow-xl rounded-[2rem] sm:rounded-[2.5rem] border border-transparent hover:border-slate-100 transition-all duration-300"
              >
                <div className={`w-12 h-12 sm:w-14 sm:h-14 ${benefit.color} rounded-2xl flex items-center justify-center`}>
                  <benefit.icon size={isMobile ? 24 : 28} />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-slate-900">{benefit.title}</h3>
      ...
      <section id="download" className="py-16 md:py-24 bg-slate-900 relative overflow-hidden rounded-[2rem] md:rounded-[3rem] lg:rounded-[4rem] mx-4 md:mx-12 mb-16 md:mb-24">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center text-white px-6">
          <h2 className="text-3xl md:text-6xl font-extrabold mb-6 md:mb-8">Ready to transform your learning?</h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <a 
              ...
              className="w-full sm:w-auto bg-white text-slate-900 px-10 py-4 sm:py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-4 hover:bg-slate-50 transition-all shadow-2xl active:scale-95 group"
            >
              <img src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Google Play" className="h-8" />
            </a>
            <div className="text-slate-500 text-xs sm:text-sm font-semibold tracking-wide flex items-center gap-2">
        ...
        <div className="max-w-5xl mx-auto px-6 relative z-10">
          ...
          >
            ...
            <form className="space-y-10">
              ...
              <button type="button" className="w-full group relative overflow-hidden rounded-2xl bg-slate-900 text-white py-4 sm:py-5 font-bold text-base sm:text-lg shadow-[0_20px_40px_rgba(15,23,42,0.1)] transition-all hover:shadow-[0_25px_50px_rgba(15,23,42,0.2)] hover:-translate-y-1">
