          ...
          >
            {displayedMentors.map((mentor) => (
              ...
              >
                ...
                <div className="px-2">
                  <div className="flex items-center gap-2 mb-1.5 text-brand-500">
                  </div>
                  ...
                  {/* Expertise Dropdown - Tighter & More Integrated */}
                  <div className="mb-4">
                    <button 
                      onClick={() => setExpandedId(expandedId === mentor.id ? null : mentor.id)}
                      className="flex items-center gap-2 py-1.5 px-3 rounded-full bg-slate-50 hover:bg-brand-50 transition-colors group/btn border border-slate-100/50"
                    >
                      ...
                      <ChevronDown 
                        size={14} 
                        className={`text-slate-400 group-hover/btn:text-brand-400 transition-transform duration-300 ${expandedId === mentor.id ? 'rotate-180' : ''}`} 
                      />
                    </button>
                        ...
                        >
                          <div className="pt-3 flex flex-wrap gap-1.5">
                            {mentor.expertise?.map((exp, i) => (
                              <span key={i} className="text-[8.5px] font-bold px-2.5 py-1 bg-white text-slate-600 rounded-md border border-slate-100 shadow-sm">
                                {exp}
                              </span>
                            ))}
                          </div>
